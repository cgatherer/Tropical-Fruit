...
function checkHeader(toogle){
      if(typeof toggle == 'undefined'){
            if(parseInt(window.innerHeight) < 520){
                  // console.log('windowheight');
                  var collapse = window.innerHeight - 111;
                  jQuery('.nav-collapse').css({ "max-height": collapse + 'px' });
                  jQuery('.nav-collapse').css('overflow', 'auto');
            }
      }
}
!function($) {
...
!function($) {
    ...
    $(document).on("click.collapse.data-api", "[data-toggle=collapse]", function(e) {
        ...
        if ($(this).hasClass("collapsed") && parseInt(window.innerHeight) < 520){
             jQuery('.nav-collapse').css({ "max-height":  '' });
      }else if(!$(this).hasClass("collapsed") && parseInt(window.innerHeight) < 520){
            checkHeader($(this).hasClass("collapsed"));
      }
    });
}(window.jQuery);
